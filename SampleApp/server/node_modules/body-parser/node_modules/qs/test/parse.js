...
describe('#parse', function() {
  ...
  it('parses a simple string', function(done) {
    ...
    expect(Qs.parse('0=foo')).to.deep.equal({
      '0': 'foo'
    });
    expect(Qs.parse('foo=c++')).to.deep.equal({
      foo: 'c  '
    });
    expect(Qs.parse('a[>=]=23')).to.deep.equal({
      a: {
        '>=': '23'
      }
    });
    ...
    expect(Qs.parse('foo')).to.deep.equal({
      foo: ''
    });
    expect(Qs.parse('foo=bar')).to.deep.equal({
      foo: 'bar'
    });
  ...
  it('parses a single nested string', function(done) {
    ...
    expect(Qs.parse('a[b]=c')).to.deep.equal({
      a: {
        b: 'c'
      }
    });
  ...
  it('parses a double nested string', function(done) {
    ...
    expect(Qs.parse('a[b][c]=d')).to.deep.equal({
      a: {
        b: {
          c: 'd'
        }
  ...
  it('defaults to a depth of 5', function(done) {
    ...
    expect(Qs.parse('a[b][c][d][e][f][g][h]=i')).to.deep.equal({
      a: {
        b: {
          c: {
            d: {
  ...
  it('only parses one level when depth = 1', function(done) {
    ...
    expect(Qs.parse('a[b][c]=d', 1)).to.deep.equal({
      a: {
        b: {
          '[c]': 'd'
        }
    ...
    expect(Qs.parse('a[b][c][d]=e', 1)).to.deep.equal({
      a: {
        b: {
          '[c][d]': 'e'
        }
  ...
  it('parses a simple array', function(done) {
    ...
    expect(Qs.parse('a=b&a=c')).to.deep.equal({
      a: ['b', 'c']
    });
    done();
  });
  ...
  it('parses an explicit array', function(done) {
    ...
    expect(Qs.parse('a[]=b&a[]=c&a[]=d')).to.deep.equal({
      a: ['b', 'c', 'd']
    });
    done();
  });
  ...
  it('parses a nested array', function(done) {
    ...
    expect(Qs.parse('a[b][]=c&a[b][]=d')).to.deep.equal({
      a: {
        b: ['c', 'd']
      }
    });
    expect(Qs.parse('a[>=]=25')).to.deep.equal({
      a: {
        '>=': '25'
      }
    });
  ...
  it('allows to specify array indices', function(done) {
    ...
    expect(Qs.parse('a[1]=c&a[0]=b&a[2]=d')).to.deep.equal({
      a: ['b', 'c', 'd']
    });
    expect(Qs.parse('a[1]=c&a[0]=b')).to.deep.equal({
      a: ['b', 'c']
    });
    expect(Qs.parse('a[1]=c')).to.deep.equal({
      a: ['c']
    });
    done();
  });
  ...
  it('limits specific array indices to 20', function(done) {
    ...
    expect(Qs.parse('a[20]=a')).to.deep.equal({
      a: ['a']
    });
    expect(Qs.parse('a[21]=a')).to.deep.equal({
      a: {
        '21': 'a'
      }
    });
  ...
  it('supports encoded = signs', function(done) {
    ...
    expect(Qs.parse('he%3Dllo=th%3Dere')).to.deep.equal({
      'he=llo': 'th=ere'
    });
    done();
  });
  ...
  it('is ok with url encoded strings', function(done) {
    ...
    expect(Qs.parse('a[b%20c]=d')).to.deep.equal({
      a: {
        'b c': 'd'
      }
    });
    expect(Qs.parse('a[b]=c%20d')).to.deep.equal({
      a: {
        b: 'c d'
      }
    });
  ...
  it('allows brackets in the value', function(done) {
    ...
    expect(Qs.parse('pets=["tobi"]')).to.deep.equal({
      pets: '["tobi"]'
    });
    expect(Qs.parse('operators=[">=", "<="]')).to.deep.equal({
      operators: '[">=", "<="]'
    });
    done();
  });
  ...
  it('transforms arrays to objects', function(done) {
    ...
    expect(Qs.parse('foo[0]=bar&foo[bad]=baz')).to.deep.equal({
      foo: {
        '0': 'bar',
        bad: 'baz'
      }
    });
    expect(Qs.parse('foo[bad]=baz&foo[0]=bar')).to.deep.equal({
      foo: {
        bad: 'baz',
        '0': 'bar'
      }
    });
    expect(Qs.parse('foo[bad]=baz&foo[]=bar')).to.deep.equal({
      foo: {
        bad: 'baz',
        '0': 'bar'
      }
    });
    expect(Qs.parse('foo[]=bar&foo[bad]=baz')).to.deep.equal({
      foo: {
        '0': 'bar',
        bad: 'baz'
      }
  ...
  it('correctly prunes undefined values when converting an array to an object', function(done) {
    ...
    expect(Qs.parse('a[2]=b&a[99999999]=c')).to.deep.equal({
      a: {
        '2': 'b',
        '99999999': 'c'
      }
  ...
  it('doesn\'t produce empty keys', function(done) {
    ...
    expect(Qs.parse('_r=1&')).to.deep.equal({
      '_r': '1'
    });
    done();
  });
  ...
  it('parses arrays of objects', function(done) {
    ...
    expect(Qs.parse('a[0][b]=c')).to.deep.equal({
      a: [{
        b: 'c'
      }]
    });
  ...
  it('allows for empty strings in arrays', function(done) {
    ...
    expect(Qs.parse('a[]=b&a[]=&a[]=c')).to.deep.equal({
      a: ['b', '', 'c']
    });
    expect(Qs.parse('a[0]=b&a[1]=&a[2]=c&a[19]=')).to.deep.equal({
      a: ['b', '', 'c', '']
    });
    done();
  });
  ...
  it('compacts sparse arrays', function(done) {
    ...
    expect(Qs.parse('a[10]=1&a[2]=2')).to.deep.equal({
      a: ['2', '1']
    });
    done();
  });
  ...
  it('should not throw when a native prototype has an enumerable property', {
    parallel: false
  }, function(done) {
